...
if ( $choice_box eq "" ) {
 say
'<a href="http://adam.vbi.vt.edu"/>ADAM has moved.</a> Please update your bookmarks';
 exit 1;
}
...
sub create_input_function {
 ...
 if ( $format_box eq 'oPDS' ) {
  ...
  if ($upload_file) {
   ...
   open( TA, ">$funcfilename" )
     or die("<br>ERROR: Cannot open the file for functions! <br>");
   print "open ok \n<br>" if ($DEBUG);
   ...
   print
"<br>ERROR: There must be input functions. Please upload a file for functions or enter the functions directly into the text area. <br>";
   exit;
  }
  ...
  if ($external_parameters) {
   open( EP, ">$epfilename" )
     or die("<br>ERROR: Cannot open the file for external parameters! <br>");
   print "open ok \n<br>" if ($DEBUG);
   print EP $external_parameters;
   close(EP)
     or die("<br>ERROR: Cannot close the file for external parameters! <br>");
  }
  else {
   print
"<br>ERROR: There must be some external parameters for the system. Please enter the external parameters directly into the text area. <br>";
   exit;
  }
...
sub set_update_type() {
 ...
 if ( $update_box eq 'Sequential' ) {
  ...
  {
   $update_schedule =~
     s/^\s+|\s+$//g;    #remove all leading and trailing white spaces
   $update_schedule =~
     s/(\d+)\s+/$1 /g;    # remove extra spaces in between the numbers
...
sub SDDSerrorchecking_and_set_flags {
 ...
 if ( $initialState eq "\"\"" ) {
  print
"<br> ERROR: The initial state must be specified for the system. Please check the initial state. <br>";
  exit;
 }
 if ( $interestingNodes eq "\"\"" ) {
  print
"<br> ERROR: The nodes of interest must be specified for the system. Please check the nodes of interest. <br>";
  exit;
 }
 if ( $num_states eq "" ) {
  print
"<br> ERROR: The number of states must be specified for the system. Please check the number of states. <br>";
  exit;
 }
 if ( $num_steps eq "" ) {
  print
"<br> ERROR: The number of steps must be specified for the system. Please check the number of states. <br>";
  exit;
 }
 if ( $num_simulations eq "" ) {
  print
"<br> ERROR: The number of simulations must be specified for the system. Please check the number of states. <br>";
  exit;
 }
  ...
  given ($format_box) {
   ...
   when (/SDDS/) {
    ...
    if ($upload_file) {
     ...
     print
"<br>ERROR: There must be a file uploaded for the (complete) transition table or functions. <br>";
     exit;
    }
    ...
    if ($propensityMatrix) {
     ...
     open( PM, ">$filename_pm" )
       or
       die("<br>ERROR: Cannot open the file for propensity parameters! <br>");
     print PM $propensityMatrix;
     close(PM)
       or
       die("<br>ERROR: Cannot close the file for propensity parameters! <br>");
    }
    else {
    ...
    if ($DEBUG) {
     say(
"<br> perl SDDS.pl -f $filename -p $filename_pm -i $initialState -n $interestingNodes -s $num_states -e $num_steps -m $num_simulations -a $flag4ss -b $flag4tm -g $plot_file -h $histogram_file -t $tm_file <br>"
     );
    }
    system(
"perl SDDS.pl -f $filename -p $filename_pm -i $initialState -n $interestingNodes -s $num_states -e $num_steps -m $num_simulations -a $flag4ss -b $flag4tm -g $plot_file -h $histogram_file -t $tm_file"
    );
   }    # end of /SDDS/
   ...
   when (/oSDDS/) {
    ...
    if ($external_parameters) {
     ...
     print
"<br>ERROR: There must be some external parameters for the (open) system. Please enter the external parameters directly into the text area. <br>";
     exit;
    }
    ...
    if ($DEBUG) {
     say(
"perl SDDS.pl -f $filename -p $filename_pm -i $initialState -n $interestingNodes -s $num_states -e $num_steps -m $num_simulations -a $flag4ss -b $flag4tm -g $plot_file -h $histogram_file -t $tm_file <br>"
     );
    }
    system(
"perl SDDS.pl -f $filename -p $filename_pm -i $initialState -n $interestingNodes -s $num_states -e $num_steps -m $num_simulations -a $flag4ss -b $flag4tm -g $plot_file -h $histogram_file -t $tm_file"
...
"<font color=red>Please enter a length of the limit cycle you wish to compute. Enter 1 for fixed points</font>";
  die(
"Program quitting. Empty field entered for limit cycle length in large networks."
  );
 }
...
"<br><A href=\"$clientip.out1.$DGformat\" target=\"_blank\"><font color=\"#226677\"><i>Click to view the dependency graph.</i></font></A><br>";
 ...
 print
   "ruby analysisByGroebnerBasis.rb $n_nodes $p_value $filename $limCyc_length"
   if ($DEBUG);
...
"<font color=red>Simulation for large networks is not possible. Please chose <i>Algorithms</i> as <b>Analysis</b> option. </font><br>";
  ...
  if ( $option_box eq "All trajectories from all possible initial states" ) {
   ...
   {
    print
"Calculating fixed points and limit cycles, not generating a graph of the state space.<BR><pre>\n";
    $output =
      system(
"php ./modules/StateSpaceAnalysis/cyclonePDS.php $p_value ${clientip}.functionfile"
      );

    # print($output);
    $output =
      system(
"php ./modules/StateSpaceAnalysis/graphAnalysis.php ${clientip}.functionfile"
      );
...
"<font color=blue><b>Computing Trajectory of the given initialization</b></font> <br>";
    ...
    && ( $trajectory_value ne "" ) )
   {
    ...
    $trajectory_value =~
      s/(\d+)\s+/$1 /g;    #remove extra white space between the numbers
...
"<br><font color=red>Please enter an initial state or select <i>Complete State Space</i></font><br>";
  ...
  if ($statespace eq "State space graph"
   && $option_box eq "All trajectories from all possible initial states" )
  {
 ...
 if ( -e "$clientip.plot.png" ) {
  print
"<br><A href=\"$clientip.plot.png\" target=\"_blank\"><font color=\"#226677\"><i>Click here to see the plot of cell population simulation.</i></font></A><br>";
 }
 if ( -e "$clientip.histogram.png" ) {
  print
"<br><A href=\"$clientip.histogram.png\" target=\"_blank\"><font color=\"#226677\"><i>Click here to see the histogram for probability distribution.</i></font></A><br>";
 }
 if ( -e "$clientip.tm.txt" ) {
  print
"<br><A href=\"$clientip.tm.txt\" target=\"_blank\"><font color=\"#226677\"><i>Click here to see the probability transition matrix of the system.</i></font></A><br>";
 }
}

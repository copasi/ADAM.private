...
use DVDCore
  qw($Clientip $Function_data $Function_file &error_check @Output_array $Pwd &dvd_session &_log $Stochastic);
use Cwd;
use Getopt::Std;
getopts('vh');
...
$all_trajectories_flag =
  $ARGV[2];    # This flag set means all possible arrows are drawn in one

# graph, without this option, one update is choosen at random for every
# variables
$update_stochastic_flag =
  $ARGV[3];    #if set, an update stochastic system is simulated using
...
$trajectory_flag =
  $ARGV[10]
  ; # 1 if all trajectories, 0 for a single trajetory form intitial state trajectory_value
...
@response = dvd_session(
 $n_nodes,               $p_value,                $clientip,
 $translate,             $update_sequential_flag, $update_schedule,
 $all_trajectories_flag, $statespace,             $ss_format,
 $regulatory,            $dg_format,              $trajectory_flag,
 $trajectory_value,      $update_stochastic_flag, $DEBUG,
 $function_file
);

if ( $response[0] == 1 )
{    # a response code should always be returned by the main DVDCore functions

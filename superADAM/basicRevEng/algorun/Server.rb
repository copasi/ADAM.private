	...
	case request.path
		when "/do/run"
			...
			if ((not check_input) || (check_input && validate_input(json))) then
				begin
					input_file=ENV["CODE_HOME"]+"/input.json"
					output_file=ENV["CODE_HOME"]+"/output.json"
					executable_file=ENV["CODE_HOME"]+"/src/find-pds"
					File.open(input_file,'w') { |f| f.write(JSON.generate(json)) }	
					system('/usr/bin/M2script '+executable_file+' < '+input_file+' > '+output_file)
